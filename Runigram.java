...
public class Runigram {
	...
	private static void print(Color[][] image) {
		//// Replace this comment with your code Notice that all you have to so is print
		/// every element (i,j) of the array using the print(Color) function.
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				int newRow = i * row / height;
				int newCol = j * col / width;
				newImage[i][j] = image[newRow][newCol];
			}
		}
		return newImage;
	}
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		int vr = (int) (alpha * c1.getRed() + (1 - alpha) * c2.getRed());
		int vg = (int) (alpha * c1.getGreen() + (1 - alpha) * c2.getGreen());
		int vb = (int) (alpha * c1.getBlue() + (1 - alpha) * c2.getBlue());
